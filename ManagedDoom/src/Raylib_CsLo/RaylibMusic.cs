...
{
    public sealed class RaylibMusic : IMusic, IDisposable
    {
        ...
        public RaylibMusic(Config config, Wad wad, string sfPath, int audioBufferSize)
        {
            try
            {
                ...
                stream = new MusStream(this, config, sfPath, audioBufferSize);
                current = Bgm.NONE;
        ...
        private class MusStream : IDisposable
        {
            ...
            public MusStream(RaylibMusic parent, Config config, string sfPath, int audioBufferSize)
            {
                this.parent = parent;
                this.config = config;
                this.audioBufferSize = audioBufferSize;
                ...
                left = new float[audioBufferSize];
                right = new float[audioBufferSize];
                batch = new short[2 * audioBufferSize];

                audioStream = Raylib.LoadAudioStream((uint)MusDecoder.SampleRate, 16, 2);
            }
            ...
            public void SetDecoder(IDecoder decoder)
            {
                ...
                if (!Raylib.IsAudioStreamPlaying(audioStream))
                {
            ...
            public unsafe void Update()
            {
                ...
                for (var t = 0; t < audioBufferSize; t++)
                {
                ...
                fixed (void* p = batch)
                {
                    Raylib.UpdateAudioStream(audioStream, p, audioBufferSize);
                }
            }
