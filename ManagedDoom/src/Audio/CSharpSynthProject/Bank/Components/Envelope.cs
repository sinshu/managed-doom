...
namespace AudioSynthesis.Bank.Components
{
    ...
    public class Envelope
    {
        ...
        public void QuickSetup(int sampleRate, int note, short keyNumToHold, short keyNumToDecay, float susMod, EnvelopeDescriptor envelopeInfo)
        {
            ...
            //Decay
            stages[3].offset = envelopeInfo.SustainLevel * susMod;
            stages[3].scale = envelopeInfo.PeakLevel - envelopeInfo.SustainLevel * susMod;
            stages[3].time = Math.Max(0, (int)(sampleRate * (envelopeInfo.DecayTime) * Math.Pow(2, ((60 - note) * keyNumToDecay) / 1200.0)));
            stages[3].graph = Tables.EnvelopeTables[envelopeInfo.DecayGraph];
            //Sustain
            stages[4].offset = 0;
            stages[4].scale = envelopeInfo.SustainLevel * susMod;
            stages[4].time = (int)(sampleRate * envelopeInfo.SustainTime);
            //Release
            stages[5].offset = 0;
            stages[5].scale = stages[3].time == 0 && stages[4].time == 0 ? envelopeInfo.PeakLevel : stages[4].scale;
