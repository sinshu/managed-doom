namespace AudioSynthesis.Midi
{
    ...
    public class MidiFile
    {
        ...
        private MidiTrack MergeTracks()
        {
            ...
            List<byte> channelsUsed = new List<byte>();
            ...
            for (int x = 0; x < mTracks.Length; x++)
            {
                ...
                foreach (byte p in mTracks[x].ActiveChannels)
                {
                    if (!channelsUsed.Contains(p))
                        channelsUsed.Add(p);
                }
            }
            MidiTrack track = new MidiTrack(programsUsed.ToArray(), drumprogramsUsed.ToArray(), channelsUsed.ToArray(), new MidiEvent[eventCount]);
        ...
        private void LoadStream(BinaryReader reader)
        {
            if (!FindHead(reader, 500))
                throw new InvalidDataException("Invalid midi file : MThd chunk could not be found.");
            ReadHeader(reader);
            for (int x = 0; x < mTracks.Length; x++)
            {
        ...
        private static MidiTrack ReadTrack(BinaryReader reader)
        {
            ...
            long endPosition = ReadInt(reader) + reader.BaseStream.Position;
            byte prevStatus = 0;
            ...
            if (reader.BaseStream.Position != endPosition)
                throw new InvalidDataException("The track length was invalid for the current MTrk chunk.");
            if (channelList.Contains(MidiHelper.DrumChannel))
            {
            ...
            MidiTrack track = new MidiTrack(instList.ToArray(), drumList.ToArray(), channelList.ToArray(), eventList.ToArray());
        ...
        private static MidiEvent ReadMetaMessage(BinaryReader reader, int delta, byte status)
        {
            ...
            switch (metaStatus)
            {
                case 0x0://sequence number
                    {
                        ...
                        if (count == 0)
                            ...
                            throw new InvalidDataException("Invalid sequence number event.");
                    }
                case 0x1://text
                ...
                case 0x20://midichannel
                    if (reader.ReadByte() != 1)
                        throw new InvalidDataException("Invalid midi channel event. Expected size of 1.");
                    return new MetaEvent(delta, status, metaStatus, reader.ReadByte());
                case 0x21://midiport
                    if (reader.ReadByte() != 1)
                        throw new InvalidDataException("Invalid midi port event. Expected size of 1.");
                    return new MetaEvent(delta, status, metaStatus, reader.ReadByte());
                case 0x2F://endoftrack
                    return new MetaEvent(delta, status, metaStatus, reader.ReadByte());
                case 0x51://tempo
                    if (reader.ReadByte() != 3)
                        throw new InvalidDataException("Invalid tempo event. Expected size of 3.");
                ...
                case 0x54://smpte
                    if (reader.ReadByte() != 5)
                        throw new InvalidDataException("Invalid smpte event. Expected size of 5.");
                ...
                case 0x58://time sig
                    if (reader.ReadByte() != 4)
                        throw new InvalidDataException("Invalid time signature event. Expected size of 4.");
                    return new MetaTextEvent(delta, status, metaStatus, reader.ReadByte() + ":" + reader.ReadByte() + ":" + reader.ReadByte() + ":" + reader.ReadByte());
                case 0x59://key sig
                    if (reader.ReadByte() != 2)
                        throw new InvalidDataException("Invalid key signature event. Expected size of 2.");
            ...
            throw new InvalidDataException("Not a valid meta message Status: " + status + " Meta: " + metaStatus);
        }
        private static MidiEvent ReadRealTimeMessage(BinaryReader reader, int delta, byte status)
        {
            switch (status)
            {
                ...
                default:
                    throw new InvalidDataException("The real time message was invalid or unsupported : " + status);
            }
        }
        private static MidiEvent ReadSystemCommonMessage(BinaryReader reader, int delta, byte status)
        {
            switch (status)
            {
                ...
                default:
                    throw new InvalidDataException("The system common message was invalid or unsupported : " + status);
            }
        }
        ...
        private static int TrackVoiceStats(MidiEvent midiEvent, List<byte> instList, List<byte> drumList, List<byte> channelList, int noteOnCount)
        {
            if (midiEvent.Command == 0x90)
            {
                byte chan = (byte)midiEvent.Channel;
                if (!channelList.Contains(chan))
                    channelList.Add(chan);
                noteOnCount++;
            }
            else if (midiEvent.Command == 0xC0)
            {
                byte chan = (byte)midiEvent.Channel;
                byte prog = (byte)midiEvent.Data1;
                if (chan == MidiHelper.DrumChannel)
                {
                    if (!drumList.Contains(prog))
                        drumList.Add(prog);
                }
                else
                {
                    if (!instList.Contains(prog))
                        instList.Add(prog);
                }
            }
            return noteOnCount;
