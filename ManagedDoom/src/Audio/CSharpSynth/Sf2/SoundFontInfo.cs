    ...
    public class SoundFontInfo
    {
        ...
        public SoundFontInfo(BinaryReader reader)
        {
            ...
            if(!id.ToLower().Equals("list"))
                throw new InvalidDataException("Invalid soundfont. Could not find INFO LIST chunk.");
            long readTo = reader.BaseStream.Position + size;
            id = new string(IOHelper.Read8BitChars(reader, 4));
            if (!id.ToLower().Equals("info"))
                throw new InvalidDataException("Invalid soundfont. The LIST chunk is not of type INFO.");
            while (reader.BaseStream.Position < readTo)
            {
                ...
                switch (id.ToLower())
                {
                    ...
                    case "isft":
                        ...
                        break;
                    default:
                        throw new InvalidDataException("Invalid soundfont. The Chunk: " + id + " was not expected.");
                }
            }
        }
