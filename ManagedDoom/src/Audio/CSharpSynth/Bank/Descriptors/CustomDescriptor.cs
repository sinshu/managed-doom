namespace AudioSynthesis.Bank.Descriptors
{
    ...
    public class CustomDescriptor : IDescriptor
    {
        ...
        public void Read(string[] description)
        {
            ...
            if (size % 2 == 1)
                size++;
            objs = desc.Values.ToArray<object>();
        }
            ...
            while (read < size)
            {
                switch ((char)reader.ReadByte())
                {
                    ...
                    case '&':
                        ...
                        break;
                    default:
                        throw new InvalidDataException("Invalid custom descriptor: " + id);
                }
            }
            if (read > size)
                throw new InvalidDataException("Invalid custom descriptor: " + id);
        ...
        public int Write(BinaryWriter writer)
        {
            ...
            {
                throw new InvalidDataException("More bytes were written than expected.");
            }
            return written;
        }
    }
