    ...
    public class PatchBank
    {
        ...
        private void LoadMyBank(Stream stream)
        {
            using (BinaryReader reader = new BinaryReader(stream))
            {
                ...
                if (!id.Equals("info"))
                    throw new InvalidDataException("Invalid bank file. The INFO chunk is missing.");
                if (reader.ReadSingle() != BankVersion)
                    throw new InvalidDataException(string.Format("Invalid bank file. The bank version is incorrect, the correct version is {0:0.000}.", BankVersion));
                this.comment = IOHelper.Read8BitString(reader, size - 4);
                //read asset list chunk
                id = IOHelper.Read8BitString(reader, 4).ToLower();
                size = reader.ReadInt32();
                if (!id.Equals("list"))
                    throw new InvalidDataException("Invalid bank file. The ASET LIST chunk is missing.");
                ...
                while(reader.BaseStream.Position < readTo)
                {
                    ...
                    size = reader.ReadInt32();
                    if (!id.Equals("smpl"))
                        throw new InvalidDataException("Invalid bank file. Only SMPL chunks are allowed in the asset list chunk.");
                ...
                size = reader.ReadInt32();
                if (!id.Equals("list"))
                    throw new InvalidDataException("Invalid bank file. The INST LIST chunk is missing.");
                readTo = reader.BaseStream.Position + size;
                id = IOHelper.Read8BitString(reader, 4).ToLower();
                if (!id.Equals("inst"))
                    throw new InvalidDataException("Invalid bank file. The LIST chunk is not of type INST.");
                //--read instruments
                while (reader.BaseStream.Position < readTo)
                {
                    id = IOHelper.Read8BitString(reader, 4).ToLower();
                    size = reader.ReadInt32();
                    if (!id.Equals("ptch"))
                        throw new InvalidDataException("Invalid bank file. Only PTCH chunks are allowed in the instrument list chunk.");
                    ...
                    if (patchTypes.TryGetValue(patchType, out type))
                        ...
                        throw new InvalidDataException("Invalid patch type \"" + patchType + "\"! Patch ID: " + patchName);
